...
### センシティブなメディアに関するガイドライン
- 基本的にはセンシティブ（性的、グロテスク）なメディアは実写でないかつ投稿者が閲覧注意フラグを設定すればOKです
  * NieRくらいの表現ならNSFWつけてくれれば全然OK
- ただし、運営に利用しているサーバーのホスティング会社の規約や現地の法律の関係上、以下に該当するメディアは**非実写であっても**アップロードしないでください。
  * 未成年者が描写されており、下着で隠すような部分を過度に露出しているかつ修正が不十分と判断されるもの
  * 未成年者が性的な行為を行っているシーンを描写したもの（性的な行為と分かるものであればモザイク等で修正されていても不可）
  * 性犯罪に関連するシーンを性的な意図の元描写しているもの（児童誘拐を含みます）
